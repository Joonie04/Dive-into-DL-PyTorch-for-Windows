...
def load_data_wiki(batch_size, max_len):
    ...
    """
    import sys
    sys.path.insert(0, 'd:/NEXT_LEVEL/Projects/DeepLearning/codes/chapter14')
    from chapter14_9 import _WikiTextDataset, _read_wiki
...
## 14.10.1 预训练BERT
net = BERTModel(len(vocab), num_hiddens=128, norm_shape=[128], ffn_num_input=128, ffn_num_hiddens=256, num_heads=2, num_layers=2, dropout=0.2, key_size=128, query_size=128, value_size=128, hid_in_features=128, mlm_in_features=128, nsp_in_features=128)
device = try_all_gpus()
...
def _get_batch_loss_bert(net, loss, vocab_size, tokens_X, segments_X, valid_lens_X, pred_positions_X, mlm_weights_X, mlm_Y, nsp_Y):
    ...
    参数:
        ...
        segments_X: 片段索引张量
        valid_lens_X: 有效长度张量
        pred_positions_X: 预测位置张量
        mlm_weights_X: 掩蔽语言模型权重张量
        mlm_Y: 掩蔽语言模型标签张量
        nsp_Y: 下一句预测标签张量
...
print("encoded_pair_crane[0][:3]", encoded_pair_crane[0][:3])
