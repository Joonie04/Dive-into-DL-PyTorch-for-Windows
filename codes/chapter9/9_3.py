...
# 创建多层LSTM层
lstm_layer = nn.LSTM(num_inputs, num_hiddens, num_layers)  # 创建LSTM层，输入大小为词表大小，隐藏层大小为256，层数为2
# 创建RNN模型
model = RNNModel(lstm_layer, len(vocab))  # 创建RNN模型，使用LSTM层和词表大小
# 将模型移动到指定设备（GPU或CPU）
model = model.to(device)  # 将模型移动到GPU或CPU

# 设置训练参数
num_epochs, lr = 500, 2  # 设置训练轮数为500，学习率为2
# 训练模型
train_ch8(model, train_iter, vocab, lr, num_epochs, device)  # 训练多层LSTM模型
