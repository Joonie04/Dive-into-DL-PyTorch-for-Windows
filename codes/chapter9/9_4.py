# 9.4 双向循环神经网络（Bidirectional RNN）
...
class RNNModel(nn.Module):  # 定义RNN模型类，继承自nn.Module
    ...
    def begin_state(self, device, batch_size=1):  # 定义初始化状态的函数
        if not isinstance(self.rnn, nn.LSTM):  # 如果不是LSTM
            # nn.GRU以张量作为隐藏状态
            return torch.zeros((self.num_directions * self.rnn.num_layers, batch_size, self.num_hiddens), device=device)  # 返回全零张量作为初始隐藏状态
        else:  # 如果是LSTM
            # nn.LSTM以元组作为隐藏状态
            return (torch.zeros((self.num_directions * self.rnn.num_layers, batch_size, self.num_hiddens), device=device),  # 返回全零张量作为初始记忆细胞
                    torch.zeros((self.num_directions * self.rnn.num_layers, batch_size, self.num_hiddens), device=device))  # 返回全零张量作为初始隐藏状态

# 9.4.3 双向循环神经网络的错误应用
# 加载数据集
batch_size, num_steps, device = 32, 35, try_gpu()  # 设置批量大小为32，时间步数为35，尝试使用GPU
train_iter, vocab = load_data_time_machine(batch_size, num_steps)  # 加载时间机器数据集，返回数据迭代器和词表

# 通过设置bidirectional=True来定义双向循环神经网络
vocab_size, num_hiddens, num_layers = len(vocab), 256, 2  # 设置词表大小、隐藏层大小为256、层数为2
num_inputs = vocab_size  # 输入大小等于词表大小
device = try_gpu()  # 尝试使用GPU，如果不可用则使用CPU
# 创建双向LSTM层
lstm_layer = nn.LSTM(num_inputs, num_hiddens, num_layers, bidirectional=True)  # 创建LSTM层，输入大小为词表大小，隐藏层大小为256，层数为2，设置为双向
# 创建RNN模型
model = RNNModel(lstm_layer, len(vocab))  # 创建RNN模型，使用双向LSTM层和词表大小
# 将模型移动到指定设备（GPU或CPU）
...
# 训练双向LSTM模型
train_ch8(model, train_iter, vocab, lr, num_epochs, device)  # 训练双向LSTM模型

# 注意：这是一个错误的应用示例！
# 双向RNN不适合用于语言建模（自回归任务），原因如下：
# 1. 双向RNN需要看到整个序列才能计算输出，而语言建模需要根据前文预测下一个词
# 2. 在训练时，双向RNN可以利用未来信息，但在推理时无法获取
# 3. 这导致训练和推理之间存在不一致性
# 
# 双向RNN的正确应用场景：
# - 序列标注任务（如命名实体识别、词性标注）
...
# 
# 预期输出：
# perplexity 1.1, 120361.2 tokens/sec on cuda:0
# time travellererererererererererererererererererererer
# 
# 可以看到困惑度很低（1.1），但生成的文本质量很差（重复字符）
# 这是因为模型在训练时"作弊"了，看到了未来信息
