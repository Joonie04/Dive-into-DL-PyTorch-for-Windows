...
# 定义初始化GRU参数的函数
def get_params(vocab_size, num_hiddens, device):  # 定义初始化GRU参数的函数
    ...
    # 更新门参数
    W_xz = normal((num_inputs, num_hiddens))  # 输入到更新门的权重矩阵，形状为(vocab_size, num_hiddens)
    W_hz = normal((num_hiddens, num_hiddens))  # 隐藏状态到更新门的权重矩阵，形状为(num_hiddens, num_hiddens)
    b_z = torch.zeros(num_hiddens, device=device)  # 更新门的偏置，形状为(num_hiddens)
    # 重置门参数
    W_xr = normal((num_inputs, num_hiddens))  # 输入到重置门的权重矩阵，形状为(vocab_size, num_hiddens)
    W_hr = normal((num_hiddens, num_hiddens))  # 隐藏状态到重置门的权重矩阵，形状为(num_hiddens, num_hiddens)
    b_r = torch.zeros(num_hiddens, device=device)  # 重置门的偏置，形状为(num_hiddens)
    # 候选隐藏状态参数
    W_xh = normal((num_inputs, num_hiddens))  # 输入到候选隐藏状态的权重矩阵，形状为(vocab_size, num_hiddens)
    W_hh = normal((num_hiddens, num_hiddens))  # 隐藏状态到候选隐藏状态的权重矩阵，形状为(num_hiddens, num_hiddens)
    b_h = torch.zeros(num_hiddens, device=device)  # 候选隐藏状态的偏置，形状为(num_hiddens)
    # 输出层参数
    ...
    # 附上梯度
    params = [W_xz, W_hz, b_z, W_xr, W_hr, b_r, W_xh, W_hh, b_h, W_hq, b_q]  # 将所有参数组合成列表
    for param in params:  # 遍历所有参数
        param.requires_grad_(True)  # 启用梯度计算
    return params  # 返回参数列表

# 定义初始化GRU状态的函数
def init_rnn_state(batch_size, num_hiddens, device):  # 定义初始化GRU状态的函数
    return (torch.zeros((batch_size, num_hiddens), device=device), )  # 返回初始隐藏状态，形状为(batch_size, num_hiddens)

# 定义GRU前向传播函数
def gru(inputs, state, params):  # 定义GRU前向传播函数
    W_xz, W_hz, b_z, W_xr, W_hr, b_r, W_xh, W_hh, b_h, W_hq, b_q = params  # 解包参数
    H, = state  # 解包状态，H为隐藏状态
    outputs = []  # 初始化输出列表
    for X in inputs:  # 遍历每个时间步的输入
        # 计算更新门：控制前一时刻的状态信息被带入到当前状态中的程度
        Z = torch.sigmoid((X @ W_xz) + (H @ W_hz) + b_z)  # 更新门：σ(X*W_xz + H*W_hz + b_z)
        # 计算重置门：控制忽略前一时刻状态信息的程度
        R = torch.sigmoid((X @ W_xr) + (H @ W_hr) + b_r)  # 重置门：σ(X*W_xr + H*W_hr + b_r)
        # 计算候选隐藏状态：使用重置门控制前一时刻状态的影响
        H_tilda = torch.tanh((X @ W_xh) + ((R * H) @ W_hh) + b_h)  # 候选隐藏状态：tanh(X*W_xh + (R*H)*W_hh + b_h)
        # 计算新的隐藏状态：使用更新门控制新旧状态的混合
        H = Z * H + (1 - Z) * H_tilda  # 新隐藏状态：Z*H + (1-Z)*H_tilda
        # 计算输出
        Y = H @ W_hq + b_q  # 输出：H*W_hq + b_q
        outputs.append(Y)  # 将输出添加到输出列表
    return torch.cat(outputs, dim=0), (H,)  # 返回所有输出和新的隐藏状态

# 训练GRU模型（从零实现）
vocab_size, num_hiddens, device = len(vocab), 256, try_gpu()  # 设置词表大小、隐藏层大小和设备
num_epochs, lr = 500, 1  # 设置训练轮数为500，学习率为1
model = RNNModelScratch(len(vocab), num_hiddens, device, get_params, init_rnn_state, gru)  # 创建GRU模型
train_ch8(model, train_iter, vocab, lr, num_epochs, device)  # 训练GRU模型

# 9.1.3 简洁实现
# 使用PyTorch内置的GRU层
num_inputs = vocab_size  # 输入维度等于词表大小
gru_layer = nn.GRU(num_inputs, num_hiddens)  # 创建GRU层，输入维度为词表大小，隐藏层大小为256
model = RNNModel(gru_layer, len(vocab))  # 创建GRU模型
