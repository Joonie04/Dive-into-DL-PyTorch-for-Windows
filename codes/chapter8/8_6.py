...
# 8.6.1 定义模型
# 使用PyTorch内置的RNN层
num_hiddens = 256  # 设置隐藏层大小为256
rnn_layer = nn.RNN(len(vocab), num_hiddens)  # 创建RNN层，输入维度为词表大小，隐藏层大小为256
state = torch.zeros((1, batch_size, num_hiddens))  # 初始化隐藏状态，形状为(层数, 批量大小, 隐藏层大小)
print("state.shape:", state.shape)  # 打印初始状态的形状

# 测试RNN层
X = torch.rand(size=(num_steps, batch_size, len(vocab)))  # 创建随机输入，形状为(时间步数, 批量大小, 词表大小)
Y, state_new = rnn_layer(X, state)  # 前向传播
print("Y.shape:", Y.shape)  # 打印输出的形状
print("state_new.shape:", state_new.shape)  # 打印新状态的形状
...
device = try_gpu()  # 尝试使用GPU
net = RNNModel(rnn_layer, vocab_size=len(vocab))  # 创建RNN模型
net = net.to(device)  # 将模型移动到指定设备

# 测试预测函数
predict_ch8('time traveller ', 10, net, vocab, device)  # 预测10个字符

# 训练模型
num_epochs, lr = 500, 1  # 设置训练轮数为500，学习率为1
train_ch8(net, train_iter, vocab, lr, num_epochs, device)  # 训练RNN模型
