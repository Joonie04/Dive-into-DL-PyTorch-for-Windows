...
# 8.5.1 独热编码
# 测试独热编码
F.one_hot(torch.tensor([0, 2]), len(vocab))  # 对索引[0, 2]进行独热编码，词表大小为vocab的长度
print("F.one_hot(torch.tensor([0, 2]), len(vocab))", F.one_hot(torch.tensor([0, 2]), len(vocab)))  # 打印独热编码结果

# 测试独热编码的批量处理
X = torch.arange(10).reshape((2, 5))  # 创建一个张量，形状为(2, 5)
print("X", X)  # 打印X
print("F.one_hot(X.T, len(vocab))", F.one_hot(X.T, len(vocab)))  # 对X的转置进行独热编码

# 8.5.2 初始化模型参数
def get_params(vocab_size, num_hiddens, device):  # 定义初始化模型参数的函数
    ...
    # 隐藏层参数
    W_xh = normal((num_inputs, num_hiddens))  # 输入到隐藏状态的权重矩阵，形状为(vocab_size, num_hiddens)
    W_hh = normal((num_hiddens, num_hiddens))  # 隐藏状态到隐藏状态的权重矩阵，形状为(num_hiddens, num_hiddens)
    ...
    # 附上梯度
    params = [W_xh, W_hh, b_h, W_hq, b_q]  # 将所有参数组合成列表
    for param in params:  # 遍历所有参数
        param.requires_grad_(True)  # 启用梯度计算
    return params  # 返回参数列表

# 8.5.3 循环神经网络模型
def init_rnn_state(batch_size, num_hiddens, device):  # 定义初始化RNN状态的函数
    return (torch.zeros((batch_size, num_hiddens), device=device), )  # 返回初始隐藏状态，形状为(batch_size, num_hiddens)

def rnn(inputs, state, params):  # 定义RNN前向传播函数
    # inputs的形状：(时间步数量，批量大小，词向量维度)
    W_xh, W_hh, b_h, W_hq, b_q = params  # 解包参数
    H, = state  # 解包状态，H为隐藏状态
    outputs = []  # 初始化输出列表
    for X in inputs:  # 遍历每个时间步的输入
        H = torch.tanh(torch.matmul(X, W_xh) + torch.matmul(H, W_hh) + b_h)  # 计算新的隐藏状态：tanh(X*W_xh + H*W_hh + b_h)
        Y = torch.matmul(H, W_hq) + b_q  # 计算输出：Y = H*W_hq + b_q
...
# 测试RNN模型
num_hiddens = 512  # 设置隐藏层大小为512
net = RNNModelScratch(len(vocab), num_hiddens, try_gpu(), get_params, init_rnn_state, rnn)  # 创建RNN模型
state = net.begin_state(X.shape[0], try_gpu())  # 初始化RNN状态
Y, new_state = net(X.to(try_gpu()), state)  # 前向传播
print("Y.shape", Y.shape)  # 打印输出形状
print("len(new_state)", len(new_state))  # 打印新状态的长度
print("new_state[0].shape", new_state[0].shape)  # 打印新状态的形状
...
# 测试预测函数
predict_ch8('time traveller ', 10, net, vocab, try_gpu())  # 预测10个字符
...
train_ch8(net, train_iter, vocab, lr, num_epochs, try_gpu())  # 训练RNN模型

# 使用随机抽样重新训练
net = RNNModelScratch(len(vocab), num_hiddens, try_gpu(), get_params, init_rnn_state, rnn)  # 创建新的RNN模型
train_ch8(net, train_iter, vocab, lr, num_epochs, try_gpu(), use_random_iter=True)  # 使用随机抽样训练RNN模型
