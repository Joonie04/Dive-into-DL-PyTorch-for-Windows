...
net = nn.Sequential(nn.Flatten(),  # 展平层
                    nn.Linear(784, 256),  # 第一个线性层
                    nn.ReLU(),  # ReLU激活函数
                    # 第一个全连接层后面添加一个dropout层
                    nn.Dropout(0.2),  # Dropout层，dropout率为0.2
                    nn.Linear(256, 256),  # 第二个线性层
                    nn.ReLU(),  # ReLU激活函数
                    # 第二个全连接层后面添加一个dropout层
                    nn.Dropout(0.5),  # Dropout层，dropout率为0.5
                    nn.Linear(256, 10))  # 第三个线性层
