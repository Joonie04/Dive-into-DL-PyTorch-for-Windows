...
def dropout_layer(X, dropout):  # 定义dropout层的函数
    assert 0 <= dropout <= 1  # 确保dropout率在0到1之间
    if dropout == 1:  # 如果dropout率为1
        return torch.zeros_like(X)  # 返回全零张量
    if dropout == 0:  # 如果dropout率为0
        return X  # 返回原张量
    mask = (torch.rand(X.shape) > dropout).float()  # 生成随机掩码
    return mask * X / (1.0 - dropout)  # 应用dropout并缩放

X = torch.arange(16, dtype=torch.float32).reshape((2, 8))  # 创建测试张量
print(X)  # 打印原始张量
print(dropout_layer(X, 0.))  # 打印dropout率为0的结果
print(dropout_layer(X, 0.5))  # 打印dropout率为0.5的结果
print(dropout_layer(X, 1.))  # 打印dropout率为1的结果


num_inputs, num_outputs, num_hiddens1, num_hiddens2 = 784, 10, 256, 256  # 设置输入维度、输出维度和两个隐藏层的维度

dropout1, dropout2 = 0.2, 0.5  # 设置两个dropout层的dropout率

class Net(nn.Module):  # 定义神经网络类
    def __init__(self, num_inputs, num_outputs, num_hiddens1, num_hiddens2,
                 is_training=True):  # 初始化函数
        super(Net, self).__init__()  # 调用父类初始化
        self.num_inputs = num_inputs  # 保存输入维度
        self.is_training = is_training  # 保存训练标志
        self.linear1 = nn.Linear(num_inputs, num_hiddens1)  # 第一个线性层
        self.linear2 = nn.Linear(num_hiddens1, num_hiddens2)  # 第二个线性层
        self.linear3 = nn.Linear(num_hiddens2, num_outputs)  # 第三个线性层
        self.relu = nn.ReLU()  # ReLU激活函数

    def forward(self, X):  # 前向传播函数
        X = X.reshape(-1, self.num_inputs)  # 重塑输入张量
        H1 = self.relu(self.linear1(X))  # 第一层：线性变换+ReLU激活
        if self.is_training:  # 如果在训练模式
            H1 = dropout_layer(H1, dropout1)  # 应用dropout
        H2 = self.relu(self.linear2(H1))  # 第二层：线性变换+ReLU激活
        if self.is_training:  # 如果在训练模式
            H2 = dropout_layer(H2, dropout2)  # 应用dropout
        return self.linear3(H2)  # 第三层：线性变换

net = Net(num_inputs, num_outputs, num_hiddens1, num_hiddens2)  # 创建模型实例

num_epochs, lr, batch_size = 10, 0.5, 256  # 设置训练轮数、学习率和批次大小
