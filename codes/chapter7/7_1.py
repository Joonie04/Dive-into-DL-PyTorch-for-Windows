...
# 定义AlexNet网络结构
net = nn.Sequential(  # 使用nn.Sequential创建AlexNet网络
    # 第一层卷积：使用11x11的更大窗口来捕捉对象，步幅为4以减少输出的高度和宽度，输出通道数为96
    nn.Conv2d(1, 96, kernel_size=11, stride=4, padding=1), nn.ReLU(),  # 1个输入通道（灰度图像），96个输出通道，11x11卷积核，步幅4，填充1，使用ReLU激活函数
    nn.MaxPool2d(kernel_size=3, stride=2),  # 第一层最大池化：3x3窗口，步幅2
    # 第二层卷积：减小卷积窗口为5x5，使用填充为2来使得输入与输出的高和宽一致，增大输出通道数为256
    nn.Conv2d(96, 256, kernel_size=5, padding=2), nn.ReLU(),  # 96个输入通道，256个输出通道，5x5卷积核，填充2，使用ReLU激活函数
    nn.MaxPool2d(kernel_size=3, stride=2),  # 第二层最大池化：3x3窗口，步幅2
    # 使用三个连续的卷积层和较小的卷积窗口（3x3），输出通道的数量进一步增加
    nn.Conv2d(256, 384, kernel_size=3, padding=1), nn.ReLU(),  # 256个输入通道，384个输出通道，3x3卷积核，填充1，使用ReLU激活函数
    nn.Conv2d(384, 384, kernel_size=3, padding=1), nn.ReLU(),  # 384个输入通道，384个输出通道，3x3卷积核，填充1，使用ReLU激活函数
    nn.Conv2d(384, 256, kernel_size=3, padding=1), nn.ReLU(),  # 384个输入通道，256个输出通道，3x3卷积核，填充1，使用ReLU激活函数
    nn.MaxPool2d(kernel_size=3, stride=2),  # 第三层最大池化：3x3窗口，步幅2
    nn.Flatten(),  # 展平层：将多维张量展平为一维
    # 全连接层的输出数量是LeNet中的好几倍，使用dropout层来减轻过拟合
    nn.Linear(6400, 4096), nn.ReLU(),  # 第一个全连接层：输入6400，输出4096，使用ReLU激活函数
    nn.Dropout(p=0.5),  # Dropout层：丢弃率为0.5，随机丢弃50%的神经元
    nn.Linear(4096, 4096), nn.ReLU(),  # 第二个全连接层：输入4096，输出4096，使用ReLU激活函数
    nn.Dropout(p=0.5),  # Dropout层：丢弃率为0.5，随机丢弃50%的神经元
    nn.Linear(4096, 10))  # 输出层：输入4096，输出10（10个类别，因为使用Fashion-MNIST）

X = torch.randn(1, 1, 224, 224)  # 创建一个随机输入张量，形状为（批量大小，通道数，高度，宽度）
...
lr, num_epochs = 0.01, 10  # 设置学习率为0.01，训练轮次为10
train_ch6(net, train_iter, test_iter, num_epochs, lr, try_gpu())  # 开始训练AlexNet模型
