...
# 7.2.1 VGG块
def vgg_block(num_convs, in_channels, out_channels):  # 定义VGG块的函数
    layers = []  # 初始化层列表
    for _ in range(num_convs):  # 循环创建指定数量的卷积层
        layers.append(nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1))  # 添加3x3卷积层，填充为1
        layers.append(nn.ReLU())  # 添加ReLU激活函数
        in_channels = out_channels  # 更新输入通道数为输出通道数
    layers.append(nn.MaxPool2d(kernel_size=2, stride=2))  # 添加2x2最大池化层，步幅为2
    return nn.Sequential(*layers)  # 返回包含所有层的序列模块

# 7.2.2 VGG网络
conv_arch = ((1, 64), (1, 128), (2, 256), (2, 512), (2, 512))  # 定义VGG网络的卷积层架构：(卷积层数量, 输出通道数)

def vgg(conv_arch):  # 定义VGG网络的函数
    layers = []  # 初始化层列表
    in_channels = 1  # 初始输入通道数为1（灰度图像）
    # 卷积层部分
    for (num_convs, out_channels) in conv_arch:  # 遍历卷积层架构
        layers.append(vgg_block(num_convs, in_channels, out_channels))  # 添加VGG块
        in_channels = out_channels  # 更新输入通道数

    return nn.Sequential(  # 返回完整的VGG网络
        *layers, nn.Flatten(),  # 展开所有卷积层并添加展平层
        # 全连接层部分
        nn.Linear(out_channels * 7 * 7, 4096), nn.ReLU(), nn.Dropout(0.5),  # 第一个全连接层：输入为512*7*7，输出4096，使用ReLU和Dropout
        nn.Linear(4096, 4096), nn.ReLU(), nn.Dropout(0.5),  # 第二个全连接层：输入4096，输出4096，使用ReLU和Dropout
        nn.Linear(4096, 10))  # 输出层：输入4096，输出10（10个类别）

net = vgg(conv_arch)  # 创建VGG网络
X = torch.randn(size=(1, 1, 224, 224))  # 创建一个随机输入张量，形状为（批量大小，通道数，高度，宽度）
...
# 7.2.3 模型训练
ratio = 4  # 设置缩减比例为4
small_conv_arch = [(pair[0], pair[1] // ratio) for pair in conv_arch]  # 将每个VGG块的输出通道数除以4，减少模型复杂度
net = vgg(small_conv_arch)  # 创建缩减后的VGG网络
...
train_ch6(net, train_iter, test_iter, num_epochs, lr, try_gpu())  # 开始训练VGG模型
