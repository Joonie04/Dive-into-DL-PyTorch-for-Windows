...
# 6.6.1 LeNet
net = nn.Sequential(  # 创建LeNet网络
    nn.Conv2d(1, 6, kernel_size=5, padding=2), nn.Sigmoid(),  # 第一层卷积：1个输入通道，6个输出通道，5x5卷积核，填充2，使用Sigmoid激活函数
    nn.AvgPool2d(kernel_size=2, stride=2),  # 第一层平均池化：2x2窗口，步幅2
    nn.Conv2d(6, 16, kernel_size=5), nn.Sigmoid(),  # 第二层卷积：6个输入通道，16个输出通道，5x5卷积核，使用Sigmoid激活函数
    nn.AvgPool2d(kernel_size=2, stride=2),  # 第二层平均池化：2x2窗口，步幅2
    nn.Flatten(),  # 展平层：将多维张量展平为一维
    nn.Linear(16 * 5 * 5, 120), nn.Sigmoid(),  # 第一个全连接层：输入400，输出120，使用Sigmoid激活函数
    nn.Linear(120, 84), nn.Sigmoid(),  # 第二个全连接层：输入120，输出84，使用Sigmoid激活函数
    nn.Linear(84, 10))  # 输出层：输入84，输出10（10个类别）

X = torch.rand(size=(1, 1, 28, 28), dtype=torch.float32)  # 创建一个随机输入张量，形状为（批量大小，通道数，高度，宽度）
print("X.shape:", X.shape)  # 打印输入张量的形状
...
train_iter, test_iter = load_data_fashion_mnist(batch_size=batch_size)  # 加载FashionMNIST训练集和测试集
...
lr, num_epochs = 0.9, 10  # 设置学习率为0.9，训练轮次为10
