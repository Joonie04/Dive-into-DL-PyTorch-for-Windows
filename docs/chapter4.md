...
**线性函数的定义**：
- 满足 `f(x + y) = f(x) + f(y)` 和 `f(ax) = a·f(x)`
- 图像是一条直线（一维）或平面/超平面（高维）
- 数学形式：`y = wx + b`

**非线性函数的定义**：
- 不满足上述线性性质
- 图像是曲线或曲面
- 数学形式：如 `y = max(0, x)`、`y = 1/(1+e^(-x))` 等

**具体例子对比**：

| 类型 | 函数 | 图像特征 | 神经网络中的作用 |
|------|------|---------|-----------------|
| 线性 | `y = 2x + 1` | 一条直线 | 只能学习线性关系 |
| 线性 | `y = Wx + b` | 超平面 | 多层叠加仍等价于单层 |
| 非线性 | `y = ReLU(x) = max(0, x)` | 折线（在x=0处拐弯） | 引入非线性，增加表达能力 |
| 非线性 | `y = sigmoid(x)` | S形曲线 | 输出限制在(0,1)范围 |
| 非线性 | `y = tanh(x)` | S形曲线，过原点 | 输出限制在(-1,1)范围 |
...
**加入非线性激活函数后**：
```
第一层：h = ReLU(W₁x + b₁)   # 非线性！
第二层：y = W₂h + b₂
```
由于 ReLU 的存在，无法将两层合并为简单的线性形式，网络获得了**拟合任意复杂函数**的能力。

**直观理解**：
- 线性变换只能对数据进行**拉伸、旋转、平移**
- 非线性激活可以**弯曲、扭曲**决策边界
- 多次弯曲叠加，就能逼近任意复杂的形状
