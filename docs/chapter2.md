...
- `.argmax()`：返回最大值的索引
- `.argmin()`：返回最小值的索引
* 需要注意: max(), min(), mean(), std(), var(), argmax(), argmin() 需要把张量转换为float类型才能计算
...
* cat() 与 stack() 的区别：
  * cat() 是在已有的维度上拼接，不增加新的维度
  * stack() 是在新的维度上堆叠，增加一个新的维度
...
在PyTorch中，张量的维度从0开始编号。例如形状为 `(5, 4)` 的张量：
- 维度0（axis=0）：行方向，有5个元素
- 维度1（axis=1）：列方向，有4个元素

**求和与平均：**

```python
x = torch.arange(4, dtype=torch.float32)  # 向量 [0, 1, 2, 3]
print(x.sum())  # 所有元素的和，结果：6
...
print(A.sum(axis=0))  # 沿维度0（行）求和，即每列求和
# 结果：[40, 45, 50, 55]
# 计算：列0: 0+4+8+12+16=40, 列1: 1+5+9+13+17=45, ...

print(A.sum(axis=1))  # 沿维度1（列）求和，即每行求和
# 结果：[6, 22, 38, 54, 70]
# 计算：第0行: 0+1+2+3=6, 第1行: 4+5+6+7=22, ...

print(A.sum(axis=[0, 1]))  # 沿维度0和1求和，即所有元素求和
# 结果：190
...
`axis` 参数指定要压缩的维度，该维度会被"求和"掉：

| 操作 | 原始形状 | 结果形状 | 说明 |
|------|---------|---------|------|
| `A.sum(axis=0)` | `(5, 4)` | `(4,)` | 压缩维度0，每列求和 |
| `A.sum(axis=1)` | `(5, 4)` | `(5,)` | 压缩维度1，每行求和 |
| `A.sum(axis=[0, 1])` | `(5, 4)` | `()` | 压缩所有维度，标量 |
...
print(A / sum_A)  # 归一化，每行除以该行的和
```

`keepdims=True` 的作用：保持被压缩的维度为1，而不是完全移除它。
...
```
* 点积又可以叫内积，因为它是向量的一种特殊乘法。相应的还有外积. 两个向量的点积是一个标量, 表示这两个向量的相似度. 点积的结果可以用来判断两个向量是否正交(垂直)关系. 如果两个向量正交, 那么它们的点积为0. 外积的结果是一个矩阵.
...
**常用范数：**
* 范数（Norm）：向量或矩阵的长度或大小，用于衡量向量或矩阵的“大小”或“距离”。
* 范数的作用: 范数可以用来衡量向量或矩阵的大小或距离, 从而在优化算法中用于判断是否收敛. 例如, 在梯度下降法中, 我们需要根据梯度的大小来更新参数, 而范数就可以用来计算梯度的大小.
- L2范数：向量元素的平方和的平方根
- L1范数：向量元素的绝对值之和
...
# 定义切线方程：在x=1处的切线，斜率=导数f'(1)=2
tangent_line = 2*x - 3

# 创建x坐标点：从0到3，步长0.1，共30个点
x = np.arange(0, 3, 0.1)

# 绘制：Y参数包含两条曲线
# - f(x): 计算每个x点的函数值（蓝色实线）
# - 2*x-3: 计算每个x点的切线值（红色虚线）
...
| 参数 | 含义 | 示例 |
|------|------|------|
| `X` | x轴坐标数组 | `[0.0, 0.1, 0.2, ..., 2.9]` |
| `Y` | y轴数据列表 | `[f(x), 2*x-3]` 两条曲线 |
| `legend` | 图例标签 | `['f(x)', 'Tangent line (x=1)']` |
| `xlabel` | x轴标签 | `'x'` |
| `ylabel` | y轴标签 | `'f(x)'` |

**结果：**
- 蓝色实线：函数 `f(x) = 3x² - 4x` 的图像
- 红色虚线：在 `x=1` 处的切线 `y = 2x - 3`

**切线方程推导：**
- 函数在 `x=1` 处的导数：`f'(x) = 6x - 4`，所以 `f'(1) = 2`
- 切线方程：`y - f(1) = f'(1)(x - 1)`，即 `y - (-1) = 2(x - 1)`
- 化简：`y = 2x - 3`
...
这个函数 `f(a)` 本质上是一个线性函数，即 `f(a) = k * a` 的形式，其中 k 是某个常数。

推导过程：
1. 初始操作：`b = a * 2` → b = 2a
2. while循环：每次 `b = b * 2`，假设循环执行 n 次，则 b = 2a × 2^n = 2^(n+1) × a
3. if条件：
   - 如果 `b.sum() > 0`：`c = b` → c = 2^(n+1) × a
   - 如果 `b.sum() <= 0`：`c = 100 * b` → c = 100 × 2^(n+1) × a

无论走哪个分支，最终结果都是 `d = k * a` 的形式。
...
可以通过以下代码验证自动计算的梯度是否正确：

```python
print(a.grad == d / a)  # 验证自动计算的梯度是否等于理论梯度 d/a
```

这个验证说明PyTorch的自动微分机制对于包含控制流（while循环和if条件）的复杂函数也能正确计算梯度。
